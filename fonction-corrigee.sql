CREATE OR REPLACE FUNCTION public.creer_facture_et_abonnement_apres_paiement(p_paiement_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- 7. ✅ CORRECTION : Vérifier si une facture existe déjà via paiement_id (PAS via (SELECT id FROM factures WHERE paiement_id = p_paiement_id LIMIT 1))
