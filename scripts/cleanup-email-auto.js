/**
 * Script de nettoyage automatique d'un email (sans confirmation)
 * Usage: node scripts/cleanup-email-auto.js <email>
 * 
...
async function cleanupEmailAuto(email) {
...
if (!email) {
  console.error('❌ Usage: node scripts/cleanup-email-auto.js <email>');
  console.error('   Exemple: node scripts/cleanup-email-auto.js user@example.com');
  console.error('\n⚠️  ATTENTION: Cette opération est irréversible !');
  process.exit(1);
}
