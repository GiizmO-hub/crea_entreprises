...
/**
 * Script pour corriger automatiquement l'erreur v_paiement.facture_id
 * 
 * Ce script applique directement le fix SQL pour corriger la fonction
 * creer_facture_et_abonnement_apres_paiement qui essaie d'accéder à
 * v_paiement.facture_id (qui n'existe pas).
 */
...
console.log('═══════════════════════════════════════════════════════════');
console.log('🔧 FIX AUTOMATIQUE : Erreur v_paiement.facture_id');
console.log('═══════════════════════════════════════════════════════════\n');

// SQL pour corriger la fonction
const fixSQL = `
...
BEGIN
  ...
  -- 7. ✅ CORRECTION : Vérifier si une facture existe déjà via paiement_id (PAS via v_paiement.facture_id)
  ...
  IF v_espace_membre_id IS NULL THEN
    BEGIN
      ...
      EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE '[creer_facture_et_abonnement_apres_paiement] ⚠️ Colonne abonnement_id n''existe pas, ignoré';
      END;
...
async function main() {
  try {
    ...
    if (error) {
      ...
      console.log('⚠️  Méthode exec_sql non disponible, utilisation de la méthode alternative...\n');
      
      // Utiliser directement la connexion PostgreSQL via Supabase
      // Note: Cette méthode nécessite d'utiliser le client PostgreSQL directement
      // Pour l'instant, on va utiliser une approche différente
      
      console.log('📝 Instructions pour appliquer le fix manuellement:\n');
      console.log('1. Ouvre Supabase Dashboard → SQL Editor → New Query');
      console.log('2. Copie-colle le contenu du fichier APPLY_FIX_FACTURE_ID_NOW.sql');
      console.log('3. Clique sur "Run"\n');
      
      // Mais on peut quand même essayer d'appliquer via une requête directe
      // En utilisant le endpoint REST de Supabase pour exécuter du SQL
      ...
      if (!response.ok) {
        // Si ça ne fonctionne pas, on affiche les instructions
        console.log('❌ Impossible d\'appliquer automatiquement le fix.\n');
        console.log('📋 SOLUTION MANUELLE:\n');
        console.log('Le fichier APPLY_FIX_FACTURE_ID_NOW.sql contient le SQL à exécuter.');
        console.log('Ouvre-le et copie-colle son contenu dans Supabase Dashboard → SQL Editor.\n');
        process.exit(1);
      }
    }
    
    console.log('✅ Fix appliqué avec succès !\n');
    console.log('🔄 Tu peux maintenant relancer: node scripts/forcer-workflow-complet.mjs\n');
