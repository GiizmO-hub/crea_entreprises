/**
 * Script de nettoyage avec SERVICE_ROLE_KEY (bypass les permissions)
 * Usage: node scripts/cleanup-email-service.js <email>
 */
...
// Essayer d'utiliser SERVICE_ROLE_KEY si disponible, sinon utiliser ANON_KEY
...
async function cleanupEmailService(email) {
  console.log(`\n🧹 Nettoyage automatique de l'email: ${email}\n`);
  ...
  try {
    ...
    // Procéder au nettoyage automatiquement
    ...
    if (cleanupResult.success) {
      ...
      if (cleanupResult.details) {
        console.error('Détails:', cleanupResult.details);
      }
    }
...
if (!email) {
  console.error('❌ Usage: node scripts/cleanup-email-service.js <email>');
  console.error('   Exemple: node scripts/cleanup-email-service.js user@example.com');
  console.error('\n⚠️  ATTENTION: Cette opération est irréversible !');
  process.exit(1);
}

cleanupEmailService(email);
