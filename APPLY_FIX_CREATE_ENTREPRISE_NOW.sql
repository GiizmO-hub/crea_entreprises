-- ============================================================================
-- FIX IMMÉDIAT : create_complete_entreprise_automated avec TOUS les paramètres
-- ============================================================================
-- 
-- INSTRUCTIONS:
-- 1. Copier TOUT ce fichier
-- 2. Aller dans Supabase Dashboard → SQL Editor → New Query
-- 3. Coller le contenu
-- 4. Cliquer sur "Run"
-- 
-- ============================================================================

-- Supprimer l'ancienne version si elle existe (toutes les signatures possibles)
DROP FUNCTION IF EXISTS create_complete_entreprise_automated(text, text, text, text, text, text, text, text, numeric, text, text, text, text, text, text, text, text, text, text, text, text, uuid, uuid[], boolean, boolean) CASCADE;
DROP FUNCTION IF EXISTS create_complete_entreprise_automated(text, text, text, text, text, text, text, text, numeric, text, text, text, text, text, text, text, text, text, text, uuid, uuid[], boolean, boolean) CASCADE;

-- Créer la fonction avec la signature EXACTE que le frontend attend
...
BEGIN
  ...
  IF p_plan_id IS NOT NULL THEN
    ...
    -- Récupérer les colonnes du plan
    SELECT 
  ...
  -- 4. Créer l'entreprise AVEC statut_paiement et les nouveaux champs
...
-- Vérification
SELECT '✅ Fonction create_complete_entreprise_automated créée avec succès avec TOUS les paramètres (p_code_ape, p_code_naf, p_convention_collective)' as resultat;
