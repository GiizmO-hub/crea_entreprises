/*
  ============================================================================
  APPLICATION COMPLÈTE DES CORRECTIONS + TEST
  ============================================================================
  
  Ce script applique TOUTES les corrections nécessaires et teste le workflow.
  
  Corrections appliquées:
  1. Migration 67 : Retire statut_paiement de INSERT INTO factures
  2. Migration 68 : Récupère entreprise_id depuis les notes si NULL
...
-- ============================================================================
-- CORRECTION COMPLÈTE : creer_facture_et_abonnement_apres_paiement
-- ============================================================================
  ...
  IF v_paiement.notes IS NOT NULL AND v_paiement.notes != '' THEN
    BEGIN
      ...
      IF v_paiement.entreprise_id IS NULL AND v_notes_json ? 'entreprise_id' THEN
        ...
        WHERE id = p_paiement_id;
        SELECT * INTO v_paiement FROM paiements WHERE id = p_paiement_id;
        RAISE NOTICE '✅ Entreprise ID récupéré depuis notes et mis à jour dans le paiement: %', v_entreprise_id_from_notes;
      END IF;
    ...
    EXCEPTION
      WHEN OTHERS THEN
        RAISE WARNING '⚠️ Erreur parsing notes: %', SQLERRM;
    END;
  END IF;
...
BEGIN
  ...
  SELECT valider_paiement_carte_immediat(v_paiement_id, 'test_stripe_after_all_fixes') INTO v_result;
...
SELECT '✅ Corrections appliquées et test terminé !' as resultat;
