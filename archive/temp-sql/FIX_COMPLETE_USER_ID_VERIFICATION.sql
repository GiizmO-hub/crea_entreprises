-- ============================================================================
-- CORRECTION COMPLÈTE - Vérification user_id dans create_complete_entreprise_automated
-- ============================================================================

-- Cette correction améliore la fonction pour vérifier que le user_id existe
-- AVANT de créer l'entreprise, évitant ainsi l'erreur entreprises_user_id_fkey
...
BEGIN
  ...
  IF v_user_id IS NULL THEN
    RETURN jsonb_build_object(
      ...
      'hint', 'Verifiez que vous etes bien connecte dans l''application.'
    );
  END IF;
  
  -- ==========================================================================
  -- ÉTAPE 2: ✅ CRITIQUE - Vérifier que le user_id existe dans auth.users
  -- ==========================================================================
  -- Récupérer les informations de l'utilisateur pour vérifier son existence
  SELECT id, raw_user_meta_data->>'role' INTO v_auth_user_id, v_role
  FROM auth.users WHERE id = v_user_id;
  
  -- Si l'utilisateur n'existe pas, v_auth_user_id sera NULL
  IF v_auth_user_id IS NULL THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'Utilisateur non trouve dans auth.users. Session expiree peut-etre.',
      'hint', 'Veuillez vous reconnecter.',
      'user_id_provided', v_user_id::text
    );
  END IF;
  
  -- ==========================================================================
  -- ÉTAPE 3: Vérification supplémentaire (déjà faite ci-dessus via STRICT)
  -- ==========================================================================
  -- Si nous arrivons ici, l'utilisateur existe et est valide
  
  -- ==========================================================================
  -- ÉTAPE 4: Vérifier le plan si fourni
  -- ==========================================================================
  IF p_plan_id IS NOT NULL THEN
    ...
    IF v_plan_montant_mensuel IS NULL OR v_plan_montant_mensuel = 0 THEN
      RETURN jsonb_build_object(
        'success', false,
  ...
  -- ==========================================================================
  -- ÉTAPE 6: Créer l'entreprise (maintenant que user_id est vérifié)
  -- ==========================================================================
  BEGIN
    ...
    VALUES (
      v_user_id, -- ✅ Garanti valide maintenant
      p_nom_entreprise,
      p_forme_juridique,
      p_siret,
      p_email_entreprise,
  ...
  EXCEPTION
    WHEN foreign_key_violation THEN
      RETURN jsonb_build_object(
        'success', false,
        'error', 'Erreur de clé étrangère: Le user_id n''existe pas dans auth.users.',
        'user_id', v_user_id::text,
        'hint', 'Votre session a peut-être expiré. Veuillez vous reconnecter et réessayer.'
      );
    WHEN OTHERS THEN
      RETURN jsonb_build_object(
        'success', false,
  ...
  -- ==========================================================================
  -- ÉTAPE 7: Créer le client si email fourni
  -- ==========================================================================
  ...
  -- ==========================================================================
  -- ÉTAPE 8: Créer un paiement si plan fourni
  -- ==========================================================================
  IF p_plan_id IS NOT NULL AND v_montant_ttc > 0 THEN
    ...
    VALUES (
      ...
      jsonb_build_object(
        ...
        'description', 'Paiement pour création entreprise: ' || p_nom_entreprise
      )::text
    )
...
EXCEPTION
  WHEN foreign_key_violation THEN
    RETURN jsonb_build_object(
      ...
      'hint', 'Vérifiez que tous les IDs fournis sont valides.'
    );
  WHEN OTHERS THEN
    RETURN jsonb_build_object(
      'success', false,
...
COMMENT ON FUNCTION create_complete_entreprise_automated IS 
  'Crée une entreprise complète avec client et paiement. Vérifie que le user_id existe AVANT création pour éviter les erreurs de clé étrangère.';

-- ============================================================================
-- VÉRIFICATION
-- ============================================================================

SELECT '✅ Fonction create_complete_entreprise_automated corrigée avec vérification du user_id' as resultat;
