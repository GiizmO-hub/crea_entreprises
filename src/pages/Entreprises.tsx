...
import { Plus, Building2, Edit, Trash2, X, Eye, EyeOff, Users, Phone, Mail, MapPin, Calendar, PlusCircle, UserX } from 'lucide-react';
...
export default function Entreprises() {
  ...
  const checkIfClient = async () => {
    if (!user) {
      console.log('⚠️ Pas d\'utilisateur pour vérification client');
      return;
    }
    
    try {
      console.log('🔍 Vérification si utilisateur est client:', user.id, user.email);
      ...
      console.log('📦 Espace client trouvé:', espaceClient, 'Erreur:', error);
      
      if (error || !espaceClient) {
        console.log('ℹ️ Utilisateur n\'est pas un client (ou pas d\'espace membre)');
        setIsClient(false);
        return;
      }
      
      console.log('✅ Utilisateur est un client, entreprise_id:', espaceClient.entreprise_id);
      setIsClient(true);
  ...
  const loadEntreprises = async () => {
    ...
    try {
      console.log('🔄 Chargement des entreprises pour user:', user.id, user.email);
      ...
      if (error) {
        console.error('❌ Erreur Supabase chargement entreprises:', error);
        throw error;
      }
      
      console.log('✅ Entreprises chargées:', data?.length || 0, data);
      setEntreprises(data || []);
    } catch (error) {
