/*
  # Fix ULTRA ROBUST: Erreur "query returned more than one row" dans create_abonnement_complete
  
  PROBLÈME:
  - L'erreur persiste malgré toutes les corrections
  - Il peut y avoir plusieurs clients avec le même ID (impossible mais vérifions)
  - Il peut y avoir plusieurs espaces membres pour le même client
  
  SOLUTION:
  - Utiliser UNIQUEMENT des agrégations (MAX, MIN, COUNT) pour garantir une seule valeur
  - Éviter complètement SELECT ... INTO sauf avec agrégation
  - Utiliser des CTEs (Common Table Expressions) pour clarifier
*/
...
BEGIN
  ...
  -- Récupérer entreprise_id et email du client (utiliser MAX pour garantir une seule valeur)
  SELECT 
    MAX(entreprise_id),
    MAX(email)
  INTO v_entreprise_id, v_client_email
  FROM clients
  ...
  -- Récupérer le user_id du client depuis l'espace membre (utiliser MAX pour garantir une seule valeur)
...
COMMENT ON FUNCTION create_abonnement_complete IS 'Créer un abonnement complet - VERSION ULTRA ROBUSTE avec MAX() pour éviter "more than one row"';
