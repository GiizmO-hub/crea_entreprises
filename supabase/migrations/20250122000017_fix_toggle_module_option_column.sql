/*
  # Fix: Correction de la fonction toggle_module_option pour gérer toutes les structures
  
  Le problème: La table abonnements peut avoir user_id, client_id, ou être liée via entreprise_id
  Solution: Créer une version simplifiée qui cherche via l'utilisateur connecté
*/

-- Recréer la fonction toggle_module_option avec la bonne logique
...
BEGIN
  ...
  -- Pour super admin, utiliser le user_id fourni ou auth.uid()
  ...
  -- Chercher selon différentes structures possibles (client_id, user_id, ou via entreprise_id)
  -- On fait plusieurs tentatives dans l'ordre de probabilité
  
  -- Tentative 1: Chercher via client_id (si existe)
  BEGIN
    SELECT a.id INTO v_abonnement_id
    FROM abonnements a
    WHERE a.statut = 'actif'
      AND a.client_id = v_target_user_id
  ...
  -- Tentative 2: Chercher via user_id (si client_id n'a pas fonctionné)
  ...
  -- Tentative 3: Chercher via entreprise_id (structure de base)
  IF v_abonnement_id IS NULL THEN
    SELECT a.id INTO v_abonnement_id
    FROM abonnements a
...
COMMENT ON FUNCTION toggle_module_option IS 'Activer/desactiver une option pour un abonnement (support client_id, user_id et entreprise_id)';
