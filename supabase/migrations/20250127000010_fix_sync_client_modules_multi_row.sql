/*
  # FIX : sync_client_modules_from_plan - Erreur "query returned more than one row"
  
  PROBLÈME:
  - La fonction sync_client_modules_from_plan échoue avec "query returned more than one row"
  - Cela se produit quand il y a plusieurs espaces membres ou abonnements
  - Résultat : les modules ne sont pas synchronisés, seul 5 modules au lieu de 42 sont présents
  
  SOLUTION:
  - Ajouter LIMIT 1 et gestion des erreurs
  - Forcer la synchronisation de tous les modules du plan
  - Corriger la logique pour éviter les multiples lignes
*/
...
-- Créer la version corrigée avec p_espace_id
...
BEGIN
  ...
  -- 1. Récupérer le client_id et l'abonnement depuis l'espace membre (avec LIMIT strict)
  ...
  -- 2. Récupérer le plan_id depuis l'abonnement (utiliser le plus récent si plusieurs)
  ...
  v_modules_count := jsonb_object_keys(v_modules_json);
  ...
  -- Compter les modules correctement
  SELECT count(*) INTO v_modules_count
  FROM jsonb_each_text(v_modules_json);
...
BEGIN
  ...
  FROM jsonb_object_keys(v_modules_json);
  
  RAISE NOTICE '[sync_client_modules_from_plan] ✅ Modules synchronisés: % modules', v_count;
...
COMMENT ON FUNCTION sync_client_modules_from_plan(uuid) IS 
  'Synchronise les modules d''un espace client depuis son plan d''abonnement. Version corrigée avec gestion des erreurs multi-row.';

COMMENT ON FUNCTION sync_client_modules_from_plan(uuid, uuid) IS 
  'Synchronise les modules d''un client avec son plan d''abonnement. Version corrigée avec gestion des erreurs multi-row.';

-- Fonction pour forcer la synchronisation de tous les clients
CREATE OR REPLACE FUNCTION sync_all_clients_modules()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, auth
...
BEGIN
  RAISE NOTICE '═══════════════════════════════════════════════════════════';
  RAISE NOTICE '  🔄 SYNCHRONISATION DE TOUS LES CLIENTS';
  RAISE NOTICE '═══════════════════════════════════════════════════════════';
  ...
  FOR v_client_record IN
    ...
    INNER JOIN abonnements a ON a.client_id = emc.user_id
    WHERE a.statut = 'actif'
      AND emc.actif = true
    ORDER BY emc.created_at DESC
  LOOP
    BEGIN
      IF v_client_record.plan_id IS NOT NULL AND v_client_record.espace_id IS NOT NULL THEN
        -- Utiliser la version avec espace_id
        v_sync_result := sync_client_modules_from_plan(v_client_record.espace_id);
        
        IF (v_sync_result->>'success')::boolean = true THEN
          v_success_count := v_success_count + 1;
          RAISE NOTICE '✅ Client % synchronisé (% modules)', 
            v_client_record.client_id, 
            v_sync_result->>'modules_count';
        ELSE
          v_error_count := v_error_count + 1;
          v_errors := array_append(v_errors, 
            format('Client %s: %s', v_client_record.client_id, v_sync_result->>'error'));
        END IF;
      END IF;
    EXCEPTION
      WHEN OTHERS THEN
        v_error_count := v_error_count + 1;
        v_errors := array_append(v_errors, 
          format('Client %s: %s', v_client_record.client_id, SQLERRM));
    END;
  END LOOP;
  ...
  RAISE NOTICE '═══════════════════════════════════════════════════════════';
  RAISE NOTICE '   ✅ Réussis: %', v_success_count;
...
SELECT '✅ Migration de correction sync_client_modules_from_plan appliquée avec succès !' as resultat;
