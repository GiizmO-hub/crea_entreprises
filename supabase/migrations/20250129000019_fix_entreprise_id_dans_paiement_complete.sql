/*
  # CORRECTION FINALE : Récupération entreprise_id depuis le paiement
  
  Problème :
  - L'entreprise_id n'est pas récupéré depuis le paiement
  - La fonction creer_facture_et_abonnement_apres_paiement retourne "ID d'entreprise manquant"
  
  Solution :
  - Vérifier que create_complete_entreprise_automated stocke bien entreprise_id dans la colonne
  - Améliorer creer_facture_et_abonnement_apres_paiement pour récupérer entreprise_id depuis l'entreprise créée
*/

-- ========================================
-- PARTIE 1 : Vérifier et améliorer la récupération d'entreprise_id
-- ========================================
...
BEGIN
  ...
  -- 12. ✅ CORRECTION : Générer le numero de facture AVANT l'INSERT
  ...
  -- 13. Créer la facture avec le numero
  ...
  -- 13. Créer l'abonnement si plan_id fourni
  ...
  -- 16. Mettre à jour le rôle du client dans la table utilisateurs
...
COMMENT ON FUNCTION creer_facture_et_abonnement_apres_paiement IS 
  'Crée automatiquement la facture, l''abonnement et l''espace membre client après un paiement. Version améliorée qui cherche entreprise_id dans plusieurs sources : colonne paiement, notes, user_id.';

DO $$
BEGIN
  RAISE NOTICE '✅ Migration 20250129000019 appliquée';
  RAISE NOTICE '📋 creer_facture_et_abonnement_apres_paiement améliorée pour récupérer entreprise_id depuis plusieurs sources';
END $$;
