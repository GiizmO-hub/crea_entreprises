/*
  # CORRECTION COMPLÈTE : Workflow de création d'entreprise
  
  PROBLÈMES IDENTIFIÉS:
  1. create_complete_entreprise_automated (20250131000016) ne crée PAS workflow_data
  2. creer_facture_et_abonnement_apres_paiement attend workflow_data mais il n'existe pas
  3. statut_paiement n'est pas défini lors de la création de l'entreprise
  4. Incohérence entre les différentes versions des fonctions
  
  SOLUTION:
  - Unifier create_complete_entreprise_automated pour créer workflow_data
  - S'assurer que statut_paiement est correctement défini
  - Garantir que toutes les données nécessaires sont stockées
*/
...
BEGIN
  ...
  -- 4. Créer l'entreprise AVEC statut_paiement
  ...
  VALUES (
    ...
    'active', v_statut_paiement,  -- ✅ CORRECTION : Ajouter statut_paiement
...
'Crée une entreprise et un client. Si un plan est sélectionné, crée le paiement ET workflow_data pour garantir le bon fonctionnement du workflow. CORRECTION: Définit statut_paiement lors de la création de l''entreprise.';

SELECT '✅ Migration de correction complète du workflow de création d''entreprise appliquée avec succès !' as resultat;
