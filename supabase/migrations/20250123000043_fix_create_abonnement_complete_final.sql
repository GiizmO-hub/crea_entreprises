/*
  # Fix FINAL: Erreur "query returned more than one row" dans create_abonnement_complete
  
  PROBLÈME:
  - Toutes les requêtes SELECT ... INTO doivent garantir une seule ligne
  - Même avec LIMIT 1, PostgreSQL peut lever une erreur si plusieurs lignes sont trouvées avant le LIMIT
  
  SOLUTION:
  - Utiliser des sous-requêtes pour TOUTES les requêtes SELECT ... INTO
  - Ou utiliser MAX/MIN/agrégation pour garantir une seule valeur
...
BEGIN
  ...
  -- Vérifier que le client existe et récupérer son entreprise_id et email
  RAISE NOTICE '🔍 [create_abonnement_complete] Recherche du client...';
  
  -- Utiliser une sous-requête pour garantir une seule ligne
  SELECT entreprise_id, email INTO v_entreprise_id, v_client_email
  FROM (
    SELECT entreprise_id, email
    FROM clients
    WHERE id = p_client_id
    LIMIT 1
  ) sub;

  IF v_entreprise_id IS NULL AND v_client_email IS NULL THEN
  ...
  IF v_entreprise_id IS NULL THEN
    ...
    ELSE
      -- Utiliser la première entreprise de l'utilisateur connecté
      -- Utiliser une sous-requête avec MAX pour garantir une seule valeur
  ...
  -- Récupérer le user_id du client depuis l'espace membre (pour client_id dans abonnement)
  -- Utiliser une sous-requête pour éviter "more than one row"
  ...
  IF v_client_user_id IS NULL AND v_client_email IS NOT NULL THEN
    -- Utiliser MAX(id) pour garantir une seule valeur même si plusieurs users ont le même email
  ...
  -- Utiliser COALESCE avec MAX pour garantir une seule valeur
...
COMMENT ON FUNCTION create_abonnement_complete IS 'Créer un abonnement complet avec plan, options et prix personnalisé - VERSION FINALE avec toutes les corrections "more than one row"';
