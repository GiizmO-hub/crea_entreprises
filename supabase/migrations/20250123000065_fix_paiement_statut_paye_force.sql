/*
  # CORRECTION : Forcer le statut "payé" avant de créer facture et abonnement
  
  PROBLÈME:
  - Le paiement reste en "en_attente" même après un paiement Stripe réussi
  - creer_facture_et_abonnement_apres_paiement vérifie que statut = 'paye' avant de continuer
  - Si le paiement n'est pas marqué "payé", le workflow s'arrête à 60%
  
  SOLUTION:
  - Modifier creer_facture_et_abonnement_apres_paiement pour forcer le statut "payé" si nécessaire
  - Permettre la création même si le statut est "en_attente" (assumant que si on appelle cette fonction, c'est que le paiement est réussi)
*/

-- ============================================================================
-- CORRECTION : creer_facture_et_abonnement_apres_paiement
-- Forcer le statut "payé" avant de continuer
-- ============================================================================
...
COMMENT ON FUNCTION creer_facture_et_abonnement_apres_paiement IS 
  'Crée automatiquement facture, abonnement, espace client avec droits admin après validation d''un paiement. Version corrigée qui force le statut "payé" si nécessaire et parse les notes JSONB avec fallbacks pour plan_id.';
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'creer_facture_et_abonnement_apres_paiement') THEN
    RAISE NOTICE '✅ Fonction creer_facture_et_abonnement_apres_paiement mise à jour - Force statut "payé" si nécessaire';
  END IF;
END $$;
