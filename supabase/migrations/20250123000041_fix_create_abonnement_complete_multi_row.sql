/*
  # Fix: Erreur "query returned more than one row" dans create_abonnement_complete
  
  PROBLÈME:
  - L'utilisateur a plusieurs entreprises (12 entreprises)
  - La requête SELECT id INTO v_entreprise_id FROM entreprises WHERE user_id = auth.uid() 
    peut retourner plusieurs lignes même avec LIMIT 1 si utilisé avec INTO
  - De plus, l'insertion dans abonnements manque le champ user_id qui est requis
  
  SOLUTION:
  - Utiliser une approche plus robuste pour récupérer l'entreprise
  - Ajouter le user_id dans l'insertion
  - Améliorer la logique de sélection d'entreprise
*/
...
BEGIN
  ...
  RAISE NOTICE '✅ [create_abonnement_complete] Client trouvé - Entreprise ID depuis client: %', v_entreprise_id;
  ...
  -- Le client_id dans abonnements correspond au user_id de l'espace membre client
  SELECT user_id INTO v_client_user_id
  FROM espaces_membres_clients
  WHERE client_id = p_client_id
  LIMIT 1;

  -- Si pas trouvé dans espace membre, essayer de trouver via le client directement
  IF v_client_user_id IS NULL THEN
    -- Chercher dans auth.users par email du client
    -- Utiliser une sous-requête pour éviter "more than one row"
    SELECT id INTO v_client_user_id
    FROM (
      SELECT au.id
      FROM auth.users au
      JOIN clients c ON c.email = au.email
  ...
  IF p_montant_mensuel IS NULL THEN
    SELECT montant INTO v_plan_montant
    FROM (
      SELECT 
        CASE 
...
COMMENT ON FUNCTION create_abonnement_complete IS 'Créer un abonnement complet avec plan, options et prix personnalisé - CORRIGÉ pour éviter "more than one row"';
