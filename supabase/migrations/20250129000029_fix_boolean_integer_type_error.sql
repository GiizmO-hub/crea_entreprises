/*
  # CORRECTION : Erreur de type boolean > integer
  
  Problème :
  - Les variables v_role_updated et v_auth_role_updated étaient déclarées comme boolean
  - GET DIAGNOSTICS ... = ROW_COUNT retourne un integer
  - La comparaison IF v_role_updated > 0 échouait car boolean > integer n'existe pas
  
  Solution :
  - Changer le type de v_role_updated et v_auth_role_updated de boolean à integer
  - Initialiser à 0 au lieu de false
  - Les comparaisons > 0 fonctionneront maintenant
*/
...
DECLARE
  ...
  v_role_updated integer := 0;  -- ✅ CORRIGÉ : integer au lieu de boolean
  v_auth_role_updated integer := 0;  -- ✅ CORRIGÉ : integer au lieu de boolean
BEGIN
  ...
  -- 4. Récupérer le client pour obtenir son email
  ...
  -- 5. MULTI-MÉTHODES pour récupérer auth_user_id
  ...
  -- 16. ACTIVER client_super_admin avec MULTI-MÉTHODES
  IF v_auth_user_id IS NOT NULL THEN
    BEGIN
  ...
  ELSIF v_client_email IS NOT NULL THEN
    -- Essayer même sans auth_user_id, juste avec l'email
    BEGIN
...
COMMENT ON FUNCTION creer_facture_et_abonnement_apres_paiement IS 
  'Crée automatiquement la facture, l''abonnement et l''espace membre client après un paiement. Version corrigée avec types integer pour v_role_updated et v_auth_role_updated.';

DO $$
BEGIN
  RAISE NOTICE '✅ Migration 20250129000029 appliquée';
  RAISE NOTICE '📋 Correction type boolean -> integer pour v_role_updated et v_auth_role_updated';
  RAISE NOTICE '📋 L''erreur "booléen > entier" est maintenant corrigée';
END $$;
