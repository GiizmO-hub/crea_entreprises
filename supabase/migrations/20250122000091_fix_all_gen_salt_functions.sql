/*
  # FIX COMPLET: Toutes les fonctions utilisant gen_salt
  
  PROBLÈME:
  - L'extension pgcrypto n'est pas activée
  - Plusieurs fonctions utilisent gen_salt() sans le bon search_path
  - Erreur: function gen_salt(unknown) does not exist
  
  SOLUTION:
  - Activer l'extension pgcrypto
  - Corriger TOUTES les fonctions qui utilisent gen_salt
  - S'assurer que le search_path inclut 'extensions'
*/

-- 1. Activer l'extension pgcrypto (OBLIGATOIRE)
CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- 2. Fix create_espace_membre_from_client_unified
...
BEGIN
  ...
  IF v_auth_user_id IS NULL THEN
    -- Créer l'auth user avec gen_salt depuis extensions
