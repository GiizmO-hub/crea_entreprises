/*
  # Fix : Génération automatique du mot de passe et retour systématique
  
  La fonction create_espace_membre_from_client génère déjà un mot de passe
  automatiquement si p_password est NULL ou vide, mais il n'est pas toujours
  retourné dans le résultat JSON. Ce correctif s'assure que le mot de passe
  généré est toujours retourné.
*/

-- Mettre à jour la fonction pour toujours retourner le mot de passe généré
...
COMMENT ON FUNCTION create_espace_membre_from_client IS 'Crée ou met à jour un espace membre pour un client existant avec un abonnement et des options. Génère automatiquement un mot de passe sécurisé si aucun mot de passe n''est fourni. Retourne toujours le mot de passe dans le résultat.';
