/*
  # AJOUT colonne paiement_id dans factures + Corrections finales workflow
  
  Problèmes :
  1. La colonne paiement_id peut ne pas exister dans factures
  2. Protection doublons ne fonctionne pas sans cette colonne
  3. Abonnement non créé si v_auth_user_id ou v_plan_id NULL
  
  Solutions :
  1. ✅ Ajouter colonne paiement_id dans factures (si elle n'existe pas)
  2. ✅ Créer un index unique pour éviter les doublons
  3. ✅ Améliorer la création d'abonnement avec meilleure récupération des IDs
*/

-- ========================================
-- PARTIE 1 : Ajouter colonne paiement_id dans factures si elle n'existe pas
-- ========================================
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    ADD COLUMN paiement_id uuid REFERENCES paiements(id) ON DELETE SET NULL;
    
    RAISE NOTICE '✅ Colonne paiement_id ajoutée à la table factures';
  ELSE
    RAISE NOTICE 'ℹ️ Colonne paiement_id existe déjà dans factures';
  END IF;
  
  -- Créer un index unique partiel pour éviter les doublons (un paiement = une facture)
  -- On autorise NULL car certaines factures peuvent être créées sans paiement
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes 
    WHERE tablename = 'factures' 
    AND indexname = 'idx_factures_paiement_id_unique'
  ) THEN
    CREATE UNIQUE INDEX idx_factures_paiement_id_unique 
    ON factures(paiement_id) 
    WHERE paiement_id IS NOT NULL;
    
    RAISE NOTICE '✅ Index unique créé sur factures.paiement_id';
  ELSE
    RAISE NOTICE 'ℹ️ Index unique existe déjà sur factures.paiement_id';
  END IF;
  
  -- Créer un index normal pour améliorer les performances
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes 
    WHERE tablename = 'factures' 
    AND indexname = 'idx_factures_paiement_id'
  ) THEN
    CREATE INDEX idx_factures_paiement_id ON factures(paiement_id);
    RAISE NOTICE '✅ Index créé sur factures.paiement_id';
  END IF;
END $$;

-- ========================================
-- PARTIE 2 : Recréer valider_paiement_carte_immediat avec protection renforcée
-- ========================================
...
BEGIN
  ...
  SELECT statut, id, entreprise_id, montant_ttc INTO v_paiement
  FROM paiements
  ...
  -- 2. ✅ PROTECTION RENFORCÉE : Vérifier si déjà traité (paye et facture existe)
  IF v_paiement.statut = 'paye' THEN
    -- Méthode 1 : Via paiement_id (maintenant que la colonne existe)
    SELECT id INTO v_facture_existante
    FROM factures
    WHERE paiement_id = p_paiement_id
    LIMIT 1;
    
    -- Méthode 2 : Fallback sur entreprise_id + montant + date
...
COMMENT ON FUNCTION valider_paiement_carte_immediat IS 
  'Valide un paiement par carte immédiatement. PROTECTION RENFORCÉE : vérifie via paiement_id (index unique) + fallback entreprise_id+montant+date. Évite tous les doublons.';

-- ========================================
-- PARTIE 3 : Recréer creer_facture_et_abonnement_apres_paiement avec paiement_id garanti
-- ========================================
...
BEGIN
  ...
  -- 2. ✅ PROTECTION DOUBLONS RENFORCÉE : Vérifier si une facture existe déjà via paiement_id
  -- Maintenant que la colonne existe et a un index unique, c'est la méthode principale
  SELECT id INTO v_facture_existante
  FROM factures
  WHERE paiement_id = p_paiement_id
  LIMIT 1;
  ...
  -- 13. ✅ CORRECTION : Créer la facture avec paiement_id (maintenant que la colonne existe)
  BEGIN
...
COMMENT ON FUNCTION creer_facture_et_abonnement_apres_paiement IS 
  'Crée automatiquement la facture, l''abonnement et l''espace membre client après un paiement. VERSION FINALE : Colonne paiement_id garantie dans factures avec index unique pour éviter tous les doublons.';
