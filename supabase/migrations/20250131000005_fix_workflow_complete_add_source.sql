/*
  # CORRECTION COMPLÈTE : Ajouter source='plateforme' dans creer_facture_et_abonnement_apres_paiement
  
  Problème :
  - La fonction creer_facture_et_abonnement_apres_paiement dans 20251126090019_fix_complete_workflow.sql
    ne contient PAS le champ 'source' dans l'INSERT INTO factures
  - Cela empêche la génération automatique de factures après paiement
  
  Solution :
  - Mettre à jour la fonction pour inclure source='plateforme' dans l'INSERT
*/
...
BEGIN
  ...
  -- 6. Fallback pour plan_id (chercher dans abonnements existants)
  ...
  -- 8. Si plan_id toujours NULL, erreur
  IF v_plan_id IS NULL THEN
  ...
  -- 12. ✅ CORRECTION CRITIQUE : Créer la facture AVEC source='plateforme'
  ...
  BEGIN
    ...
    ELSIF v_has_paiement_id_column THEN
      -- Cas 3 : Seulement paiement_id existe
      INSERT INTO factures (
        entreprise_id, client_id, numero, type, date_emission, date_echeance,
      ...
      RETURNING id INTO v_facture_id;
    ELSE
      -- Cas 4 : Aucune des deux colonnes n'existe
      INSERT INTO factures (
        entreprise_id, client_id, numero, type, date_emission, date_echeance,
  ...
  -- 14. Créer/Mettre à jour l'espace membre client
...
COMMENT ON FUNCTION creer_facture_et_abonnement_apres_paiement IS 
  'Crée automatiquement facture, abonnement, espace client avec droits admin après validation d''un paiement. Version corrigée avec source=''plateforme''.';
